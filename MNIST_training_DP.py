...
class Client():
    def __init__(self, dataset, filename, batch_size, model, port, train_cut=.80):
        ...
        # DataLoader数据加载器，dataset加载数据集。所有样本训练一次称为一个epoch,一个样本集被分为若干batch，训练一个batch就是一个Iteration
        ...
        # 随机梯度下降算法，lr学习率，momentum动量因子 weught_decay权重衰减
...
def run(f):
    ...
    # 这里根据端口号分配不同的数据集，模拟联邦学习的不同节点
    node = p2p.Node()
    filename1 = "./log/4/DP/loss/" + "loss_" + str(node.PORT) + ".txt"
    filename2 = "./log/4/DP/error/" + "Test_error_" + str(node.PORT) + ".txt"
    filename3 = "./log/4/DP/time/" + "time_correspond" + str(node.PORT) + ".txt"
    filename4 = "./log/4/DP/time/" + "time_compute" + str(node.PORT) + ".txt"
    ...
    for iter in range(iter_time):
        ...
        grad_with_noise = gaussian_noise(grad)
        pickling_grad = pickle.dumps(grad_with_noise)
        # pickle是python的一个标准库，实现数据的序列化和反序列化，便于数据的传输，内置基本数据结构的序列化和反序列化
        t3 = time.time()
        node.broadcast(bc_enum.SERVICE * bc_enum.DISCOVERY + bc_enum.EXCHANGEGRAD, pickling_grad)
        t4 = time.time()
        time.sleep(3)
        ...
        ### 打印通信时间和计算时间
        print('============== EPOCH=', iter, '==============')
        #print('============== time_correspond=', t_cor, '==============')
        #print('============== time_computer=', t_com, '==============')
